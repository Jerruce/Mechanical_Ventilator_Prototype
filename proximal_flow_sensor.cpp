...
/* Object definition */
I2C proximal_flow_sensor(PROXIMAL_FLOW_SENSOR_SDA_PIN, PROXIMAL_FLOW_SENSOR_SCL_PIN);

/* Global variable definition */
static float proximal_flow_slpm, tidal_volume_ml;
static char sensor_read_data_buffer[3];
static char sensor_write_data_buffer[2];
...
void Proximal_Flow_Sensor_Initialize(void){
    ...
    /* Sensor initialize */
    proximal_flow_sensor.frequency(PROXIMAL_FLOW_SENSOR_I2C_CLK_HZ);

    /* Command for resttng the sensor */
...
void Proximal_Flow_Sensor_Read(){
    ...
    if(i2c_error){
        ...
        if(i2c_error){
            error_counter++;
            if(error_counter >= PROXIMAL_FLOW_SENSOR_MAX_ERROR_COUNT){
                error_counter = 0;
                system_flags |= (1 << PROXIMAL_FLOW_SENSOR_ERROR_FLAG);
            }
        }else{
            error_counter = 0;
        }
...
uint8_t Proximal_Flow_Sensor_Send_Command(uint16_t command){
    ...
    i2c_error = proximal_flow_sensor.write(PROXIMAL_FLOW_SENSOR_SWA, sensor_write_data_buffer, 2);      
...
uint16_t Proximal_Flow_Sensor_Get_Scale_Factor(void){
    ...
    i2c_error = proximal_flow_sensor.read(PROXIMAL_FLOW_SENSOR_SRA, sensor_read_data_buffer, 2);
